...
#define configUSE_PORT_OPTIMISED_TASK_SELECTION         1                       /* 1: 使用硬件计算下一个要运行的任务,STM32支持硬件调度 0: 使用软件算法计算下一个要运行的任务, 默认: 0 ,这里选1*/
#define configUSE_TICKLESS_IDLE                         0                       /* 1: 使能tickless低功耗模式, 默认: 0 */
#define configCPU_CLOCK_HZ                              SystemCoreClock         /* 定义CPU主频, 单位: Hz, 无默认需定义,在STM32F1中是72Mhz */
#define configSYSTICK_CLOCK_HZ                          (configCPU_CLOCK_HZ / 8)/* 定义SysTick时钟频率，当SysTick时钟频率与内核时钟频率不同时才可以定义,本例程中STM32F1时钟源不一致,需要定义 单位: Hz, 默认: 不定义 */
...
#define configMINIMAL_STACK_SIZE                        128                     /* 定义空闲任务的栈空间大小, 单位: Word(256byte,0x100), 无默认需定义 */
...
/* 钩子(回调)函数相关定义 */
#define configUSE_IDLE_HOOK                             0                       /* 1: 使能空闲任务钩子(回调)函数, 无默认需定义  */
